...
import { editAnalysis } from './actions';
    ...
    return(
       <Fragment>
        <div className="symbol_container">
          ...
          <textarea autofocus id="edit_analysis" className="analysis" onChange={event => setAnalysis(event.target.value)}>{current_analysis}</textarea>
          ...
          <button id="like" onClick={() => dispatch(likeAnalysis(symbol, incoming_symbol, incoming_id))}><div className="likes"><FontAwesomeIcon icon={faArrowUp} size="sm"/> {likes}</div></button>
            ...
            <Link id="edit" to={`/${incoming_symbol}/${incoming_id}`}><button id="edit" onClick={() => dispatch(editAnalysis(symbol, incoming_symbol, incoming_id, analysis))}>Save</button></Link>
